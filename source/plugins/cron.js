...
const CRON_CONSTANTS_CONTENT_JS = `const CronExpression = {
  EVERY_SECOND: '* * * * * *',
  EVERY_5_SECONDS: '*/5 * * * * *',
  EVERY_10_SECONDS: '*/10 * * * * *',
  EVERY_30_SECONDS: '*/30 * * * * *',
...
import { CronExpression } from './cron-constants';
...
const { CronExpression } = require('./cron-constants');
...
export const initCronJobs = () => {
  cron.schedule(exampleJob.cron, exampleJob.handler);
  logger.info('Cron Jobs Initialized');
};`
      : `const cron = require('node-cron');
const exampleJob = require('./example');
const { logger } = require('../config/logger');
